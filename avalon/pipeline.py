...
### ID that is imprinted into nodes in host, allowing for easier identification. (See host.ls() and container-schema)
...
def find_config():
    ...
    if not config:
        raise EnvironmentError("No configuration found in the project nor environment")
...
def register_project(project):
    _registered_project["_"] = project

def deregister_project():
    _registered_project["_"] = None

def registered_project():
    return _registered_project["_"]
...
def register_host(host):
    ...
    Arguments:
        host (ModuleType): A module implementing the Host API interface. See the Host API
            documentation for details on what is required, or browse the source code.
    """
    signatures = {
        "ls": []
    }
...
def update_current_task(task=None, asset=None, app=None):
    ...
    changes = compute_session_changes(Session, task=task, asset=asset, app=app)
    ...
    # Emit session change
    callbacks.emit("taskChanged", changes.copy())
...
def registered_plugin_paths():
    ...
    duplicate = {superclass: paths[:] for superclass, paths in _registered_plugin_paths.items()}
    return duplicate
...
class Loader(list):
    ...
    def __init__(self, context):
        ...
        self.fname = pipeline.get_representation_path(representation)
    ...
    def load(self, context, name=None, namespace=None, options=None):
        ...
        """
        raise NotImplementedError("Loader.load() must be implemented by subclass")
    ...
    def update(self, container, representation):
        ...
        """
        raise NotImplementedError("Loader.update() must be implemented by subclass")
    ...
    def remove(self, container):
        ...
        """
        raise NotImplementedError("Loader.remove() must be implemented by subclass")
...
def create(name, asset, family, options=None, data=None):
    ...
    for Plugin in discover(Creator):
        ...
        Plugin.log.info("Creating '%s' with '%s'" % (name, Plugin.__name__))
...
def get_representation_context(representation):
    ...
    assert all([representation, version, subset, asset, project]), ("This is a bug")
...
def get_representation_path(representation):
    ...
    def path_from_representation():
        ...
        try:
            ...
            availableVars =  ", ".join(context.keys())
            log.debug("Unavailable variable used in filepath template: {0}, available variables are: {1}".format(missing_key, availableVars))
            return None

        if os.path.exists(path):
            return os.path.normpath(path)
        else:
            log.debug("path from representation doesn't exist: {}".format(path))
            return None
    ...
    def path_from_config():
        try:
            ...
            log.debug("Representation {} not found in database!".format(representation["name"]))
            return None

        try:
            template = project["config"]["template"]["publish"]
        except KeyError:
            log.debug("No publish path template in config of {}!".format(project["name"]))
            return None
        ...
        try:
            ...
            log.debug("Unavailable variable used in filepath template: {}, available variables are: {}".format(missing_key, ", ".join(data.keys())))
            return None
...
def load(Loader, representation, namespace=None, name=None, options=None, **kwargs):
    ...
    log.info("Running '%s' on '%s'" % (Loader.__name__, context["asset"]["name"]))
...
def update(container, version=-1):
    ...
    current_version, subset, asset, project = io.parenthood(current_representation)
...
def switch(container, representation):
    ...
    if not hasattr(Loader, "switch"):
        # Backwards compatibility (classes without switch support might be better to just
        # have "switch" raise NotImplementedError on the base class of Loader\
        raise RuntimeError("Loader '{}' does not support 'switch'".format(Loader.label))
...
def _validate_signature(module, signatures):
    """ Checks whether the module has the required functions/signatures."""
